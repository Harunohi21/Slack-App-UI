...
class _ShowGroupThreadState extends State<ShowGroupThread> {
  ...
  Future showThreadMessage(int channelId, int messageId) async {
    ...
    final respone = await http.post(
        ...
        body: jsonEncode(<String, dynamic>{'mention_name': mentionedNameList}));
  ...
  Widget sendGroupThreadMessageInput(int sGroupMessageId, BuildContext context,
    ...
    void handleMentionAdd(Map<String, dynamic> mentionData) {
      // Extract relevant information from mentionData
      //final String mentionId = mentionData['id'];
      //final String mentionDisplay = mentionData['display'];

      // Handle the mention as needed (e.g., add it to a list, update UI, etc.)

      mentionedNameList.add(mentionData['display']);
    }
          ...
          child: ElevatedButton(
            ...
            onPressed: () async {
              await sendGroupThreadMessage(
                  threadMsg, sGroupMessageId, mentionedNameList);
              if (status == true) {
                Navigator.pushAndRemoveUntil(
                    context,
                    MaterialPageRoute(
...
Future sendGroupThreadMessage(
    String message, int sGroupMessageId, List mentionedNameList) async {
  String? token;
  int? user_id;
  int? workspace_id;
  token = await SharedPrefUtils.getStr("token");
